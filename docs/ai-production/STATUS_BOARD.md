...
| TASK-0037 | Modern UI Localization MVP + Sprint04 Runtime Polish | QA PASS - Ready for main merge | Modern UI + Localization Unity Developer + PM | Runtime main menu, level select, HUD labels, primary buttons, level states, and language selector are modernized/localized for `tr`, `en`, `es`, `fr`, `zh-Hans`, `hi`, `nl`, `de`, `pt-BR`, `ja`, plus `it`. Sprint03 programmatic UI PNGs imported under `Assets/Resources/Generated/Sprint03/`; Sprint04 gameplay polish PNGs imported under `Assets/Resources/Generated/Sprint04/` for parallax, route arrows, hazard glow, and goal halo on all existing levels. Branch `feature/sprint04-modern-ui-localization-polish`; Unity import/compile PASS, EditMode 22/22 PASS, PlayMode 5/5 PASS. Report: `reports/modern-ui-localization-report.md`. Technical debt added: TD-0014. |
...
Modern UI localization note: `TASK-0037` and `reports/modern-ui-localization-report.md` record the MVP implementation plus Sprint04 runtime polish. The main menu and level select now use Sprint03 generated card/button/chip skins, the HUD uses a compact localized panel, and language cycling persists via `RedBall.Language`. Sprint04 adds parallax, route arrows, hazard glow, and goal halo to all 15 existing levels. Branch `feature/sprint04-modern-ui-localization-polish` passed Unity import/compile, EditMode 22/22, and PlayMode 5/5 on the current project. Technical debt added: TD-0014.

Sprint 01 technical debt note: source of truth is `docs/ai-production/technical-debt/TECHNICAL_DEBT_REGISTER.md`; TASK-0016 references existing TD-0001 through TD-0008 where relevant. Dashboard roster/state sync is still manual and remains covered by TD-0007 and TD-0008. TASK-0027 updated TD-0012 closure criteria but added no new debt ID. Post-PlayMode dashboard sync added no new debt ID. TASK-0033, TASK-0035, and TASK-0036 added no new debt ID. TASK-0037 added TD-0014 for deeper localization/string/font/manual-overflow QA.
...
| 2026-05-12 | Adopt branch-based production control: `feature/...`, `bugfix/...`, and `improvement/...` branches must carry task/decision notes, QA evidence, PM acceptance, then non-interactive merge to `main` and push. Raw asset packs are not committed wholesale; selected files are imported with provenance. | PM / Git Repo Controller |
